...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # 定义SQLite数据库路径
        db_path = "sqlite:///./data/langgraph.db"
        # 确保数据目录存在
        ...
        # 尝试创建状态图
        try:
            # 使用SQLite数据库路径和模型创建状态图
            graph = create_graph(db_path, llm_chat, llm_embedding, tool_config)
            ...
            for message_chunk, metadata in stream_data:
                try:
                    ...
                    if node_name in ["generate", "agent"]:
                        # 获取消息内容 - message_chunk 可能是 AIMessage 或字符串
